...
function logAndReturnValueBad(val: any): any {
  console.log(val);
  return val;
}
...
const resultBnt = logAndReturnValue<String>("hello");
const resultTnnt = logAndReturnValue<Number>(25);

// example 2
/* here we make a generic getRandomArrayVal, where we pass in <T> type,
that takes an array of T types, will return the T type*/
function getRandomArrayVal<T>(values: T[]): T {
...
const randomUser = getRandomArrayVal<User>(usersX);
